...
-- Ejercicios Práctica 7. Algoritmo de Prim
--
-- Alumno: José Luis Fernández Márquez
-- Titulación: 2º de Ingeniería de Computadores A
-------------------------------------------------------------------------------
...
primAux g _  [] st = st
primAux g vs rs st = primAux g (y:vs) (delete y rs) (WE x p y:st)
   where
      -- encontrar la arista de menor peso de un vértice de vs a otro de rs
      (WE x p y) = minimumBy comparaArcos [WE x p y | x <- vs, (y,p) <- successors g x, elem y rs]
      -- comparar arcos por pesos (para minimumBy)
      comparaArcos (WE _ p1 _) (WE _ p2 _) = compare p1 p2 -- Da LT EQ ó GT
